...
- **Roles:** Organisations support a four-tier role hierarchy — **Owner** (workspace & member control, organisation deletion), **Admin** (projects, members, sharing & billing), **Editor** (flags & content), and **Viewer** (read-only). See [Organisations & Users](../../guides/organisations-and-users/#roles--permissions).
...
Every change to flags (creation, update, toggle, deletion) and organisation settings (member invitations, role updates, removals, organisation deletion) is recorded in the **Audit Logs**, providing a history of who changed what and when.
